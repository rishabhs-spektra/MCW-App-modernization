...
1. In the Lab VM, from the Windows search bar, search for **Default apps** and select it.

   ![Find Default Apps](media/DefaultApp-1.png "Find Default Apps")
...
9. Select **Create project**. You may have to scroll down to see create project option.
...
10. Type **partsunlimitedweb<inject key="DeploymentID" enableCopy="false"/> (1)**  as your project name. Select **Create (2)** to continue. 
    ...
    ![Azure Migrate project settings page is shown. Project name is set to partsunlimitedweb. Create button is highlighted.](media/2-1.png "Azure Migrate Project Creation")
...
12. Another aspect of our migration project will be the database for Parts Unlimited's web site. We will have to assess the database's compatibility and migrate to Azure SQL Database. Let's switch to the **Databases (only) (1)** section in Azure Migrate. Select **Click here (2)** hyperlink for Assessment tools.
