...
5. On the **Add/Edit connection string** panel, enter the following:
   ...
   - **Deployment slot setting (3)**: Check this option to make sure connection strings stick to a deployment slot. This will make sure you can have different settings for staging and production.
   
   - Select **Apply (4)**.

     ![Add/Edit Connection string panel is open. The name field is set to StorageConnectionString. The value field is set to the connection string copied in a previous step. Type is set to Custom. The deployment slot setting checkbox is checked. The OK button is highlighted. ](media/add-connection-string.png "Deployment Slot Configuration")
