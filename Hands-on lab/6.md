...
In this task, you will learn how to deploy an Azure Function using Visual Studio Code.
You'll install required extensions, sign in to Azure, and publish your function app to the cloud.
...
4. Select **Install** to install the extensions required for your Azure Functions project. This will install C# for Visual Studio Code and Azure Functions Extension for Visual Studio Code.
    ...
    - Click on **Extension (1)** from the left panel.
    - Search **C# (2)** and select **C# (3)**.
    - Click on **Install (4)**.
...
5. Once the installation is **Finished (1)** select **Restore (2)** to download dependencies for the project.
    ...
    > **Note:** If the C# extension is already present in VS Code, it may not ask you to restore it.

6. When restore is complete, close the tabs titled **Extension (1) (2)** and the **welcome tab (3)**. Select **Azure (4)** from the left menu and select **Sign into Azure (5)** and click on **Allow** on the *"Azure resources wants to sign in..."* window. Select **Edge** as your browser if requested.
...
and select **Deploy to Function App... (4)**.
   ...
   ![Azure subscription is shown. The function app **parts-func-{uniquesuffix} (2)** is selected. On the context menu Deploy to Function App is highlighted.](media/E6T1S9-0711.png "Deploy to Function App")
...
2. Switch to the **Access keys (1)** blade under **Security + Networking** section and select **Show (2)** that is next to the Connection String value. Select the copy button for the first **connection string (3)**. Paste the value into a text editor, such as Notepad.exe, for later reference.
...
3. Go back to the resource list and navigate to your **partsunlimited-web-<inject key="DeploymentID" enableCopy="false"/> (2)**
...
4. Switch to **Environment variables (1)** blade under **Settings** section, select **Connection string (2)** and click on **+ Add (3)**.
...
8. Switch to **Environment variables (1)** blade under **Settings** section, select **App settings (2)** and  click on **+ Add (3)**.
...
9. On the **Add/Edit application setting** panel, enter the following:
   ...
   - **Value:** Enter the SQL connection string you copied in Exercise 3, Task 2, Step 2.
...
2. In the **Overview (1)** section, select **Default domain (2)** and navigate to the Parts Unlimited website hosted in Azure App Service.

   ![Parts Unlimited App Service is on screen. URL is highlighted.](media/E6T3S2-0711.png "App Service Public Endpoint")

3. Select **Login (1)**. If a login wasn't created in the earlier exercise, then select **Register as a new user? (2)**, otherwise skip to step 6 (log in to the website)
...
6. Select **Click here to Login (1)**, type the credentials listed below and click the **Login (4)** button.
...
9. Fill in sample shipping information **(1)** for testing purposes. Use coupon code **FREE (2)** and select **Submit Order (3)**. (Make sure you fill all the fields)
...
2. On the Function App Blade, select **Application Insights (1)** under **Monitoring** from the left-hand menu. On the Application Insights blade, select **Turn on Application Insights (2)**.
