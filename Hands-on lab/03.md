...
Contoso is planning to modernize its legacy on-premises web application by migrating it to the cloud to improve scalability, reliability, and operational efficiency. After completing an initial assessment and confirming compatibility, the team will use the App Service Migration Assistant to migrate the application to Azure App Service.

Once the migration is complete, the team will configure the application to securely connect to an Azure SQL Database, ensuring seamless functionality in the new cloud environment. This exercise represents a real-world scenario where Contoso transitions its application and database workloads to a fully managed Platform-as-a-Service (PaaS) solution.
...
1. On the **Add/Edit connection string** panel, enter the following:
   ...
   - **Name**: Enter `DefaultConnectionString` **(1)**
   - **Value**: Enter SQL Connection String you copied in Step 3. **(2)**
   - **Type**: Select **SQLAzure (3)**
