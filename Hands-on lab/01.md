...
Contoso Retail operates a legacy on-premises Parts Unlimited .NET web application hosted on Internet Information Services (IIS) with a backend SQL Server database. As part of its cloud modernization initiative, the company plans to migrate the application to Microsoft Azure to improve scalability, reduce infrastructure management overhead, and take advantage of managed cloud services.

In this exercise, As a Cloud Migration Engineer, you will review the existing Parts Unlimited .NET web application and its SQL Server database hosted on-premises, assess the application's readiness for migration using App Service Migration Assistant, and identify any compatibility issues before migrating the workload to Azure App Service.
...
1. From the provided VM, Click on **Start** or **Search** button, Search for **RDP (1)** and select the **Remote Desktop Connection (2)** app.
...
1. Click the **Start** button on the SQLVM. 
    ...
    > **Note**: If any IIS website, browser window, or Network Settings window is open, close it before proceeding, and then click the **Start** button.

1. In the **search box (1)**, type **SQL Server Management (2)**, then select **SQL Server Management Studio 20 (3)** from the search results.

    ![](media/sqlserver.png)

1. Connect to your on-premises SQL Database. Ensure the following details are configured correctly:

   - **Server name:** `SqlServer2008` **(1)**
   - Enable **Trust server certificate** by selecting the checkbox **(2)**.
   - Click **Connect** **(3)**.
...
1. Close all the windows and disconnect from the remote session of the SQL VM.
...
7. Minimize the **App Service Migration Assistant** window to keep it running in the background, and proceed with **Exercise 2**: Setting up **Azure Migrate** to streamline the app migration process.
