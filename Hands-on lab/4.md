...
In this task, you will learn how to connect to your SQLServer2008 VM using Remote Desktop Protocol (RDP) from your WebVM environment.
...
4. Next, click on the **Yes** button to accept the certificate and add it to trusted certificates.

   ![](media/17-04-2024(1).png)

### Task 2: Perform assessment for migration to Azure SQL Database

In this task, you will use the Microsoft Data Migration Assistant (DMA) to assess the PartsUnlimited database and identify any potential issues for migrating it to Azure SQL Database.
...
> **Note:** The Database Migration Assistant is already installed on your **SQLServer2008** VM If not found, it can be downloaded through Azure Migrate or from the [Microsoft Download Center](https://go.microsoft.com/fwlink/?linkid=2090807) as well, and as Data Migration Assistant is dependent on .NET Framework 1.8 download and install .Net Framework from [here](https://go.microsoft.com/fwlink/?LinkId=2085155) and **restart** the VM before you install Data Migration Assistant.
...
8. Now, select **Compatibility issues** to review that report as well.
...
9. Select **Upload to Azure Migrate** to upload assessment results to Azure.

    ![Upload to Azure Migrate button is highlighted.](media/dma-upload-azure-migrate.png "Azure Migrate Upload")

10. Select the right Azure environment **(1)** your subscription lives. Select **Connect (2)** to proceed to the Azure login screen. Please use the  below credentials to log in.
    ...
    * Password: <inject key="AzureAdUserPassword"></inject>
      ...
      >**Note:** If a pop-up appears regarding **Microsoft security Warning**, please click **Yes** to proceed.

      ![](media/securitywarninga.png )

11. Select your subscription **(1)** and the **partsunlimitedweb<inject key="DeploymentID" enableCopy="false"/>** Azure Migrate project **(2)**. Select **Upload (3)** to start the upload to Azure.
...
12. Once the upload is complete, select **OK**. Then, minimize the SQL VM and navigate to the Azure Migrate page on the Azure portal.

    ![Assessment Uploaded dialog shown.](media/updated43.png "Assessment Uploaded")

13. Select the **Databases (only) (1)** page on Azure Migrate. Observe the number of assessed database instances and the number of databases ready for Azure SQL DB **(2)**. Keep in mind that you might need to wait for 5 to 10 minutes for the results to show up. You can use the **Refresh** button on the page to see the latest status.
...
1. In the [Azure portal](https://portal.azure.com), navigate to your **SQL database** resource by selecting the **parts** SQL database resource from the resources list.
   ...
   ![The parts SQL database resource is highlighted in the list of resources.](media/4.3.1.png "SQL database")
...
2. On the **Overview** Blade of your SQL database, copy the **Server name** and paste the value into a text editor, such as Notepad.exe, for later reference.
   ...
   ![The server name value is highlighted on the SQL database Overview blade.](media/updated45R.png "SQL database")

### Task 4: Migrate the database schema using the Data Migration Assistant

In this task, you will migrate the database schema from SQL Server to Azure SQL Database using the Data Migration Assistant (DMA).
...
1. Return to the Data Migration Assistant in the SQL VM, and select the New **+** icon in the left-hand menu.
...
4. On the **Select target** tab, enter the following:
   ...
   - Select **Next (8)**.
      ...
      ![The Select target tab of the Data Migration Assistant is displayed, with the values specified above entered into the appropriate fields.](media/4.4.4.png "Data Migration Assistant Select target")
...
> **Note:** You can now minimize the **SQLVM** and perform the remaining exercises from the **LabVM**.

### Task 5: Migrate the database using the Azure Database Migration Service

In this task, you will migrate data from the PartsUnlimited database into an Azure SQL Database using the Azure Database Migration Service (DMS).
...
1. In the [Azure portal](https://portal.azure.com), navigate to the **hands-on-lab-<inject key="DeploymentID" enableCopy="false"/>** resource group and then select **SqlServer2008** VM.
   ...
   ![The SSMS Connect to Server dialog is displayed, with the Azure SQL Database name specified, SQL Server Authentication selected, and the demouser credentials entered.](media/01-04-2024(3)R.png "Connect to Server")
...
2. In the **SqlServer2008** VM, copy the Private IP address from the Networking section and paste it into Notepad for future reference.
   ...
   ![The SSMS Connect to Server dialog is displayed, with the Azure SQL Database name specified, SQL Server Authentication selected, and the demouser credentials entered.](media/4.5.2R.png "Connect to Server")
...
3. Navigate to your Azure Database Migration Service by selecting the **hands-on-lab-<inject key="DeploymentID" enableCopy="false"/>** resource group, and then select the **parts-dms-<inject key="DeploymentID" enableCopy="false"/>** Azure Database Migration Service from the list of resources.
   ...
   ![The contoso-dms Azure Database Migration Service is highlighted in the list of resources in the hands-on-lab-SUFFIX resource group.](media/updated51R.png "Resources")
...
5. On the New migration project blade, enter the following:

   - **Target server type:** Select **Azure SQL Database** **(1)**.

   - **Migration mode:** Select **Offline** **(2)**.

   - Click on **Configure runtime settings** **(3)**.
...
7. Click the **Search** button. In the search box, type **Microsoft Integration Runtime** **(1)** then select **Microsoft Integration Runtime** **(2)** from the search results.

   ![](media/E4T5S6.png)

8. After successful installation it will ask you for the authentication key which we copied from the Azure portal, please provide the copied key, and click on **Register** and **Finish**.
...
9. Once the Integration Runtime (Self-hosted) node has been **registered successfully**, minimize the **Microsoft Interation Runtime Configuration Manager** window.
    
   ![](media/Microsoft_Integration_Runtime_auth.png)

10. Navigate back to **Azure Database Migration Service** in the Azure portal, in **Configure integration runtime** pop-up click on **OK** **(1)**  and click on **Select** **(2)**.
...
11. On the **Source details** tab, provide the following details and click on **Next: Connect to source SQL server >> (7)** :

	- Is your Source SQL Server Instance tracked in Azure? **No (1)**
	- Source Infrastructure Type: **Virtual Machine (2)**
	- Subscription: **Select the default subscription (3)**
	- Resource Group: Select **hands-on-lab-<inject key="DeploymentID" enableCopy="false"/>** **(4)**
	- Location: **Select the default location (5)**
	- SQL server Instance Name: Enter **sqlvm<inject key="DeploymentID" enableCopy="false"/>** **(6)**

   	 ![](images/Source_detailsR.png)
...
12. On the Migration Wizard **Select source** blade, enter the following:

	- **Source server name (1):** Enter the Private IP address of SqlServer2008 that you copied in step 1.    
	...
	- **Connection properties (5):** Check both the Encrypt connection and Trust server certificate.
	   
	- Select **Next: Select databases for migration >> (6)**.
  
     	 ![The Migration Wizard Select source blade is displayed, with the values specified above entered into the appropriate fields.](media/01-04-2024(5)-1.png "Migration Wizard Select source")
   
11. Click the checkbox for **PartsUnlimited (1)** database. Select **Next: Connect to target Azure SQL Database >> (2)** to continue.
    
    ![The Migration Wizard Select database blade is displayed. PartsUnlimited databases is selected. Next: Select target >> button is highlighted.](media/Database_Selected1-1.png "Migration Wizard Select databases")

12. On the Migration Wizard **Select target** blade, enter the following:

    - **Subscription (1):** Provide the Subscription
    
    - **Resource Group (2):** Provide the Resource Group

    - **Target Azure SQL Database Server (3):** Keep it Default
    
    - **Target server name (4):** Enter the server name of your Azure SQL Database - **<inject key="sqlDatabaseName" enableCopy="false"/>.database.windows.net**
    
    - **Authentication type (5):** Select SQL Authentication.
    
    - **Username (6):** Enter **demouser**
    
    - **Password (7):** Enter **<inject key="SQLVM Password" />**
    
    - Select **Next: Map source and target databases >> (8)**.
    
      ![](media/Connecting_to_aTarget_database1-1.png )
...
13. On the Migration Wizard **Map to target databases** blade, confirm that **PartsUnlimited** is checked as the source database, and **parts** is the target database on the same line, then select **Next: Select database tables to migrate >>**.
    ...
    ![The Migration Wizard Map to target database blade is displayed, with the ContosoInsurance line highlighted.](media/01-04-2024(6)1.png "Migration Wizard Map to target databases")
...
14. On the Migration Wizard **Configure migration settings** blade, expand the **PartsUnlimited** database, verify all the tables are selected and select **Next: Database migration Summary >>**.
    ...
    > **Note:** If you see that the table data cannot be migrated, the source table is empty. It is completely fine, please select the tables which are not greyed out or the table has data in it.

    > **Note:** Click on 2 to verify the other selected databases.

    ![The Migration Wizard Configure migration settings blade is displayed, with the expand arrow for PartsUnlimited highlighted, and all the tables checked.](media/updated57new1.png "Migration Wizard Configure migration settings")
...
15. On the migration wizard **Summary** blade, select **Start migration** and monitor the migration status on the overview page of Database Migration Service at **Migration Status**.
    ...
    ![The Migration Wizard summary blade is displayed, with PartsUnlimitedDataMigration entered into the name field.](media/E4T5S17R.png "Migration Wizard Summary")

    ![The Migration Wizard summary blade is displayed, with PartsUnlimitedDataMigration entered into the name field.](media/01-04-2024(15)R.png "Migration Wizard Summary")
...
16. When the migration is complete, you should see the status as **Succeeded**.
    ...
    ![On the Migration job blade, the status of Completed is highlighted.](media/01-04-2024(15)R.png "Migration with Completed status")

### Task 6: Configure the application connection to SQL Azure Database

In this task, you will configure your application to use the SQL Azure Database, ensuring that your app can interact with the database after the migration to Azure.
...
2. Switch to the **Connection strings (1)** blade under **Settings**, and copy the connection string under **ADO.NET(SQL authentication)** by selecting the copy button **(2)**.
...
5. Switch to the **Environment variables (1)** blade under **Settings**, select **connection strings (2)** and then click on **+ Add (3).**
...
6. On the **Add/Edit connection string** panel, enter the following:
   ...
   - Select **Apply (5)**.
      ...
      ![Add/Edit Connection string panel is open. Name field is set to DefaultConnectionString. Value field is set to the connection string copied in a previous step. Type is set to SQL Azure. Deployment slot setting checkbox is checked. The OK button is highlighted. ](media/seotp.png "Adding connection string")
...
8. Switch to the **Overview (1)** blade, and select **Default domain (2)** to navigate to the Parts Unlimited website hosted in our Azure App Service using Azure SQL Database.
   ...
   ![Overview panel for the App Service is on screen. URL for the app service if highlighted.](media/appmod-ex4-t6-s8R.png "App Service public URL")
