 	...
 	- Subscription: **Select the default subscription (3)**
  	- Resource Group: Select **hands-on-lab-<inject key="DeploymentID" enableCopy="false"/>** **(4)**
   	- Location: **Select the default location (5)**
    	- SQL server Instance Name: **Provide the name (6)**
...
11. Click the checkbox for **PartsUnlimited** database. Select **Next: Connect to target Azure SQL Database >>** to continue.
    
    ![The Migration Wizard Select database blade is displayed. PartsUnlimited databases is selected. Next: Select target >> button is highlighted.](media/Database_Selected1.png "Migration Wizard Select databases")
...
12. On the Migration Wizard **Select target** blade, enter the following:
    ...
    - Select **Next: Map source and target databases >>**.
      	...
      	![](media/Connecting_to_aTarget_database1.png )
...
6. On the **Add/Edit connection string** panel, enter the following:
   ...
   - **Deployment slot setting (3)**: Check this option to make sure connection strings stick to a deployment slot. This will be helpful when we add additional deployment slots during the next exercises.
   
   - Select **Apply (4)**.

   ![Add/Edit Connection string panel is open. Name field is set to DefaultConnectionString. Value field is set to the connection string copied in a previous step. Type is set to SQL Azure. Deployment slot setting checkbox is checked. The OK button is highlighted. ](media/net3.png "Adding connection string")
