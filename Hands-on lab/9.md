...
5. On the **Basics** tab of the key vault, provide the key Vault name as **appmod-keyvault<inject key="DeploymentID" enableCopy="false"/>** **(1)**, leave all other options as default, and click **Next (2)**.

   ![](media/9.1.5R.png)

6. On the **Access Configuration** tab of the key vault, select the **Vault Access Policy (1)** in Permission Model and select the **ODL_USER_<inject key="DeploymentID"/> (2)** in Access Policies. Click on **Review + create (3)**.
...
9. Switch to the **Secrets (1)** blade under **Objects** section and select **+Generate/Import (2)**.

   ![](media/lab9_05.png)
   
10. On the Create secret panel, enter the following details, and click **Create**.
...
12. On the Secret Version panel, copy the **Secret Identifier** value and paste it into Notepad for future use.
...
4. Once the managed identity is assigned, copy the **ObjectID** and paste it into Notepad.
...
8. Under the **Principal** tab, enter the **system-assigned managed identity (1)** object ID you copied previously in step 4 and select it **(2)**. Click on **Next (3)**.
...
1. Go back to the resource list and navigate to your **partsunlimited-web-<inject key="DeploymentID" enableCopy="false"/>(2)** App Service resource. You can search for **`partsunlimited-web`** **(1)** to find your Web App.

   ![Screenshot 12](media/9.2.1.png)

2. Switch to the **Environment variables (1)** Blade, select **Connection strings (2)**, and then select the connection string with the name **DefaultConnectionString (3)**.

   ![Screenshot 13](media/E9T3S2.png)
   
3. Edit the Key Vault secret in the **@Microsoft.KeyVault(SecretUri=Secret_identifier)** format. Replace the **Secret_identifier** with the value you have copied in task 1, step 11. Copy the edited secret value that looks as mentioned below:
...
4. Select **Apply** when you get the **Save changes** pop-up, click on **Confirm**.
