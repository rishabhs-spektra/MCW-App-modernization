...
In this task, after reviewing the assessment results, you have confirmed the web application is a good candidate for migration to Azure App Service. Now we will continue migrating the application.

1. Navigate back to the **App Service Migration Assistant** window from the background. You can also click it from the bottom menu.
...
1. To continue migrating our website, Azure App Service Migration Assistant needs access to our Azure subscription. Select the **Copy Code & Open Browser** button to be redirected to the Azure portal.
...
1. Navigate to the **App Service Migration Assistant** window and select the Azure Migrate project **partsunlimitedweb<inject key="DeploymentID" enableCopy="false"/> (1)** that we created in the previous exercise to submit the results of our migration. Select **Next (2)** to continue.

    ![](media/slectamp.png)

8. To migrate the Parts Unlimited website, we must create an App Service Plan. The Azure App Service Migration Assistant will take care of all required settings.
    ...
    - Click **Migrate** **(5)** to start the migration process.
      ...
      > **WARNING:** If your migration fails with a **WindowsWorkersNotAllowedInLinuxResourceGroup (1)** error, it may be due to an incorrect region. Try the migration again and select the correct region.  
...
1. Switch to the **Environment variables (1)** Blade, select **connection string (2)**, then click **+Add (3)**.
...
1. You will be redirected to the web page that has been migrated to App Service.
